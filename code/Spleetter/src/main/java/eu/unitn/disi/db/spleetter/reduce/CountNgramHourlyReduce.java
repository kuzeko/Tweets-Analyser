...
public class CountNgramHourlyReduce extends ReduceFunction{
    private static final Log LOG = LogFactory.getLog(CountNgramHourlyReduce.class);
    ...
    public void reduce(Iterator<Record> matches, Collector<Record> records) throws Exception {
        ...
        pr2.setField(1, pr.getField(1, StringValue.class));
        pr2.setField(2, numTweets);
        records.collect(pr2);

        if (TweetCleanse.CountNgramsHourlyReduceLog) {
